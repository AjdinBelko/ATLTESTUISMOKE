...
describe("Login Test - Get created user", () => {
  ...
  it("should display correct table content and fetch contacts via API", () => {
    ...
    // table content from api response should be the same as the frontend part rendering

    cy.get('.contactTableBodyRow').each(($row, index) => { 
      cy.wrap($row).find(':nth-child(2)').should('contain.text', 'John Doe');
      cy.wrap($row).find(':nth-child(3)').should('contain.text', '1970-01-01');
      cy.wrap($row).find(':nth-child(4)').should('contain.text', 'jdoe@fake.com');
      cy.wrap($row).find(':nth-child(5)').should('contain.text', '8005555555');
      cy.wrap($row).find(':nth-child(6)').should('contain.text', '1 Main St. Apartment A');
      cy.wrap($row).find(':nth-child(7)').should('contain.text', 'Anytown KS 12345');
      cy.wrap($row).find(':nth-child(8)').should('contain.text', 'USA');
    });
